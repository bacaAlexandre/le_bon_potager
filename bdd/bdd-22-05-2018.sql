...
CREATE TABLE IF NOT EXISTS `le_bon_potager`.`T_UTILISATEURS` (
  ...
  `utiLatitude` FLOAT NOT NULL,
  `utiLongitude` FLOAT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `le_bon_potager`.`T_PRODUITS_UTILISATEURS` (
  ...
  `puDesactive` TINYINT(1) NOT NULL DEFAULT 0,
  `puDateDesactive` DATETIME NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS `le_bon_potager`.`T_LOGIN` (
  ...
  `logUtilisateur_id` INT NOT NULL,
  PRIMARY KEY (`logSelector`, `logUtilisateur_id`),
  INDEX `fk_T_LOGIN_T_UTILISATEURS1_idx` (`logUtilisateur_id` ASC),
  CONSTRAINT `fk_T_LOGIN_T_UTILISATEURS1`
    FOREIGN KEY (`logUtilisateur_id`)
    REFERENCES `le_bon_potager`.`T_UTILISATEURS` (`id_utilisateur`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
